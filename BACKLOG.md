...
| 34 | **DB hygiene: stale/zombie `pipeline_runs` rows** (pipeline DB) — zombie `36d34311` killed (→`abandoned`); 5 other stale `running` rows untriaged, check at sub-plan-4 start. Split out of #30 | Low-medium (do before sub-plan-4 validation) | 2026-06-20 |
...
- **UNTRIAGED — check at sub-plan-4 start:** 5 other stale `running` rows (13–29 days, almost certainly zombies too), NOT killed this pass: `23a6267d` (step 4, 28.7d, the abandoned pre-Section-C run), `1e834cb6` (step 3, 21.7d), `99b8f268` (step 1, 14.6d), `7dcc4794` (step 2, 13.4d), `aa81daa2` (step 6, 13.0d, the Jun-7 baseline run). Triage + likely set `abandoned` before any sub-plan-4 validation run, so they don't pollute the baseline.
