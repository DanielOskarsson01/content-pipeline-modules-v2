...
### Status — DEPLOYED + savings PROVEN on a real run (2026-06-26)

**RESOLVED.** Both halves are live in production and the cache savings are proven (not assumed). Skeleton commit `bef48ec` (`auto-21-w2-2026-06-25` branch) + content-analyzer adoption `ff28469` were deployed **Path B** (file-scoped rsync, server-only — keeps parked #29 out; the #31 gate is inside `deploy.sh` so Path B doesn't invoke it; verified #29 stayed parked) + `pm2 restart all` (4/4 fork). **Proof:** two real Anthropic calls through the deployed `content-analyzer.buildCachedPrompt` + `promptCache` + `aiStream`, same ~49K-token vocabulary prefix, different entities — **call 1 `cache_WRITE=49,050`, call 2 `cache_READ=49,050`** (uncached `tokens_in` 20 / 17). The stable prefix re-reads at ~10% input cost → ~90% saved per entity after the first in a batch. Durability: content-analyzer adoption merged to modules `main` (this entry's commit) so routine CI deploys no longer revert it.

Implementation detail of the skeleton enabling change (skeleton commit `bef48ec`):
...
**content-analyzer adoption — DONE + deployed (`ff28469`).** `buildCachedPrompt` splits the assembled prompt at `{entity_content}`: stable head (instructions + `{doc:}` vocabulary) → `cache_prefix`, variable per-entity tail → `prompt`. A runtime self-check (`cachePrefix + prompt === buildPrompt(...)`) makes it **bulletproof byte-identical** — it falls back to no-cache on the two divergence cases a `/code-review` surfaced: entity content with `$`-replacement sequences (`$$`/`$&`, which `String.replace` interprets but concat doesn't — ubiquitous in scraped text), and a template nesting `{entity_content}` in a `{doc:}` token. 23/23 tests. **Other LLM modules** (content-writer, seo-planner, tone-seo-editor) can adopt the same pattern next — each is a per-module split + byte-identity test; lower per-module value than content-analyzer's 20K-token vocab prefix. *(Latent bug noted for a deliberate follow-up: `buildPrompt`'s `String.replace` mangles `$`-sequences in scraped content — intentionally NOT changed in #21 to keep it billing-neutral.)*
...
### RESOLVED (2026-07-16 — UNIT #52, meta-output v1.0.1, branch `step5-token-economics`)
meta-output now resolves meta in **the checker's exact priority order** (direct item `meta_title`/`meta_description` fields → frontmatter → `plan.meta` legacy → `sections.meta.meta_*.candidate` → H1/first-paragraph heuristics → entity-name/empty last resort), and `assembleKeywords` mirrors `extractHeadTerms` v1.0.1 (per-section `target_keywords` + `keyword_summary_table` + `head_terms` + flat `keywords`, first plan with extractable terms). TDD: `test-meta-resolution.js` (31 assertions) includes cross-module invariant tests running the SAME fixture through meta-compliance-checker AND meta-output and asserting shipped ≡ validated — including the FIX D calibration fixture (checker-pass now implies deliverable-correct). The shared-helper de-dup across writer + checker + meta-output remains open (out of this unit's ownership — writer/checker frozen on this branch); the three readers are now behavior-aligned copies.
