...
### Session: 2026-07-15 — Step-5 token economics: caps + cache $-fix + meta-chain (branch `step5-token-economics`, NOT merged)

**Status:** Four coupled step-5 fixes built on branch `step5-token-economics` off `main` (`625b527`), test-first, `/code-review`-passed (after one CRITICAL fix), 3 commits, decision_log written, **pushed but NOT merged** (push-to-main = CI deploy; the merge is a separate deliberate act to coordinate around the Cloudinary migration + #50). Investigation STOP'd for approval before building; user approved with conditions.

**Ownership:** content-analyzer, content-writer, seo-planner, meta-compliance-checker (manifests + execute.js + tests). No skeleton edits. No hallucination-detector changes (#50's territory — file-disjoint verified; both rebase onto `625b527`, shared files are only `BACKLOG.md` + this `CLAUDE.md`, append-only).

**The four fixes (all evidence from calibration run `8801eb9e` + prove-out `f800d729`, 2026-07-14):**
- **FIX A/A+/B — max_tokens caps (commit `00e974b`).** content-analyzer 8192→16384, seo-planner 8192→16384 (both haiku-only — verified via template preset + no card override + `ai_usage` showing `claude-haiku-4-5` in loop 0 AND 1), content-writer 16384→32768. The writer runs sonnet in QA-retry cards where **adaptive thinking is ON by default and consumed max_tokens invisibly** (~10-12k thinking tokens/call, `display:"omitted"` → invisible for a month), truncating 4/7 complete round-2 retries under the skeleton's now-LIVE fail-closed guard. The brief's "writer over-generates" hypothesis was WRONG — the writer outputs markdown-only, the tokens went to thinking. Streaming verified in `stageWorker.js` (`stream:true`) so 32768 won't HTTP-timeout. `usage_notes` on analyzer/planner flag the Claude-5 thinking LANDMINE (16384 assumes haiku). `test-max-tokens.js` guards each cap.
- **Cache `$`-fix (commit `c5b0ef6`).** `buildPrompt` in all three modules inserted values via `String.prototype.replace` with a STRING replacement, interpreting `$$/$&/$`/$'/$n` — mangling scraped content AND breaking content-analyzer's prompt-cache split (prove-out showed cache_write=0/read=0 on the very Push Gaming pages that carry `$$`+`$&`). Fixed to function-form replacement (`() => value`). The split now engages byte-identically on `$`-content; `$`-free content byte-identical (no regression). `test-dollar-cache.js` per module + updated `test-cache-split.js` (its old assertion pinned the now-fixed fallback).
- **FIX D — meta:fail chain (commit `10e28cb`).** meta:fail was a batch-wide CONSTANT (100% sonnet retry): (1) seo-planner emits keywords PER SECTION + a `keyword_summary_table`, not top-level, so the checker's `extractHeadTerms` found zero → 2 keyword checks auto-failed; (2) the planner's length-valid meta candidates (`sections.meta.meta_*.candidate`) were never read — the writer fell metaTitle back to `entity.name` (11 chars) and the checker to the raw og:description (210+ chars). meta-compliance-checker v1.0.1: aggregate per-section keywords + summary table (generic `Object.values`, no section names), read the planner candidate ranked ABOVE the H1/first-paragraph heuristic. content-writer v1.6.2: `resolveMetaFromPlanner` reads the candidate; module emits `meta_title` + new `meta_description` onto the pool item. Both pipeline-agnostic. Tests prove genuine-pass AND still-fails-when-plan-absent.

**`/code-review` — one CRITICAL, fixed:** the reviewer caught that my writer docstring + README claimed **Step 8 meta-output** reads the planned meta — FALSE. `meta-output/execute.js:104-105` still reads only legacy top-level `seo_plan_json.meta.title` → falls back to `entity.name`, and `assembleKeywords` has the same per-section blindness. So after FIX D the QA gate PASSES but the SEO deliverable still ships "ELK Studios". meta-output is Step 8 — **outside this unit's ownership** — so per STOP-AND-REPORT discipline I did NOT edit it: corrected the false docs (amended into `10e28cb`) and filed **BACKLOG #52** (meta-output must read the writer's emitted fields / `sections.meta.candidate` + aggregate per-section keywords). This is the load-bearing open item — QA green ≠ artifact fixed until #52 lands.

**Filed (out-of-ownership / deferred):** #52 (meta-output loop-closure — CRITICAL, surfaced by review), #53 (skeleton `ai.complete` effort control for Claude-5 thinking — the root-cause lever FIX B could only mitigate; scope as effort-tuning not thinking-disable), #54 (cache adoption for writer/planner needs a template-prompt restructure — docs sit after `{entity_content}`, prefix below cache min), #55 (relevance-ranked top-N page selection into Step 5 — new capability; current caps blind-head-truncate).

**Token economics (calibration shape):** before ~$0.65/entity with 100% wasteful sonnet retry (meta:fail constant) + truncation-cascade failures; after ~$0.40-0.50/entity (retries fire on genuine QA fails only), truncation-cascade class eliminated, analyzer vocab head re-reads at ~10% via the engaged cache. The big remaining lever (thinking control, #53) is skeleton-owned.
