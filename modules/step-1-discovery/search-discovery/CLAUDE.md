...
4. `_partialItems` is pushed during EVERY successful page, not per entity — site_restricted fan-outs must survive mid-entity timeouts (Rule 10).

5. Run `node modules/step-1-discovery/search-discovery/test-pagination.js` after changing pagination, provider requests, stopping rules, or provenance. All HTTP and time are mocked.
