...
5. Run `node modules/step-1-discovery/ai-discovery-scout/test-ai-discovery-scout.js` after any change (mocked, free). The live LLM run is DEFERRED pending an `ANTHROPIC_API_KEY` in the build env — do it before first production use; do NOT reach into skeleton production or read secret files for the key.
