...
async function execute(input, options, tools) {
  ...
  for (let i = 0; i < entities.length; i++) {
    ...
    // add, so both drafts survive the pool. Grade only the latest (.at(-1)) --
    // the exact draft the step-8 output modules publish (markdown-output:189 /
    // html-output:182 / json-output:151) -- not both concatenated.
