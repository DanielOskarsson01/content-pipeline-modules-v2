...
> **v1.1.0 — hostname visibility + own-domain anchor (both off by default).** Two options change *what the module shows the model*, never the criteria. `include_hostname` renders each URL's full host (e.g. `casino.guru/news/...`) instead of only the path. `entity_domain_source: pool_dominant_host` derives the entity's own domain from the dominant host among its OWN-CRAWL pool items (no `search`/`scout` `found_via`) and states it as `Website:` — the anchor that was missing at step 2 (the seed website doesn't survive past step 1, so the prompt otherwise reads `Website: unknown`). Motivation: on a thin, ambiguously-named entity (Gate-1: Pocket Rockets) the model kept third-party pages about a *different* same-named company. With both on, a real haiku probe on the 322-URL Gate-1 pool dropped 18 wrong-company / publisher-furniture third-party pages (3rd-party KEEP 32→14, 3/3 runs) and kept the entity's own homepage; the derived domain was `pocketrocketsgaming.com`. Defaults reproduce the prior behavior byte-identically (proven by A/B test).
...
| `include_hostname` (boolean) | `false` | Turn on for ambiguously-named entities so the model can tell own pages from third-party pages about a different company | When on, each URL shows its full host (`casino.guru/news/...`) instead of only the path. Accepts real `true` or the string `"true"`. Off = today's path-only rendering (byte-identical) |
| `entity_domain_source` (select) | `none` | Set to `pool_dominant_host` so the model knows which host is the entity's own — pairs with `include_hostname` | `none` shows the seed website (usually `unknown` at step 2). `pool_dominant_host` derives the own domain from the dominant host among the entity's OWN-CRAWL pool items (sitemap / page-links / browser-crawler / seed — i.e. no `search`/`scout` `found_via`) and states it as `Website:`. If the own-crawl set is empty or ambiguous (a tie), it degrades **loudly** — logs a warning and leaves `Website: unknown`, never naming a wrong domain |
