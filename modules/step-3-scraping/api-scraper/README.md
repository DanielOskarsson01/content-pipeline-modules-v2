...
- **Memory (v1.0.1):** `title`, `meta_description`, `og_description`, `text_content`, and `text_preview` are trimmed regex captures / substrings of the full page HTML returned by ScrapFly. V8 keeps these as `SlicedString` views that pin the *entire* multi-MB page body alive. A `flat()` helper (`s.split('').join('')`) forces standalone byte-exact copies so the body can be garbage-collected. Output is byte-identical (proven in `test-memory.js`). Same defect class as **browser-scraper v1.1.1** (F1); aggregate impact is lower here because api-scraper only runs on the small failed-page subset
