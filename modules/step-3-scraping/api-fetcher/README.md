...
**Step 3 — Scraping (structured-source enrichment)** · `add` · `empty_ok` · cost: `medium` · v1.0.0
...
- **`auth.type`** — `none` | `query_param` (`key` → query param) | `header` (`key` → header name) | `basic` (`env` value as username, empty password — Companies House). Missing env var → provider skipped (see `skip_providers_without_auth`).
...
- `node modules/step-3-scraping/api-fetcher/test-api-fetcher.js` — 55 assertions, all HTTP mocked, no credentials, no network.
