...
- **v1.6.1 (m&a tokenizer):** `extractVocabSlugs` now treats `&` as a slug char (`/[a-z0-9&]+(?:-[a-z0-9&]+)*/`), so a legit closed-vocabulary tag like `m&a` tokenizes whole instead of splitting into `m`+`a` (both dropped by the length>=2 filter) and false-failing `vocabulary_checks`. Tests: +2 assertions in `test-vocab-fidelity.js` (23 total)
