...
- **1.5.0** -- Added an option-gated **deterministic `<think>`-block strip** (`strip_think_blocks`, default **ON**). Reasoning models run here with thinking API-disabled (`thinking:{type:'disabled'}`), so a reasoning model can write its chain-of-thought as ordinary completion text — `response.text` then flows straight to the published `content_markdown` with no strip. Both observed leaks (content-pipeline-specs Unit 4c §3b) were ONE closed `<think>...</think>` block at position 0 (9,038 and 994 words). The strip removes exactly that shape, before bracket-repair / the marker gate / stats. It **LOUD-FAILS** the entity (no draft published) on an *unclosed* `<think>` (a truncated reasoning dump) or an *all-thinking* draft (empty after strip). Default ON is byte-identical on a clean draft — no block means nothing is removed and no meta is added. See **`<think>`-leak strip** below.
- **1.4.0** -- Added an option-gated **deterministic bracket-leak repair** (`repair_bracket_leaks`, default OFF). When ON, stray body-prose bracket markers that would survive the publish strip and force-fail qa-structural Check 8 (a malformed citation like `[#-]`, a bracketed source-pointer like `[game pages]`) are deleted before the draft is graded. Repairs the mechanical leak instead of rejecting an otherwise-true, well-sourced draft (content-pipeline-specs `@94eb952` CALIBRATION_3: the writer emits a slightly different draft each run and a *different* deterministic gate catches it; repair the mechanical class, keep the substantive class gated). Detection uses the same exclusion predicates as qa-structural's `findResidualBrackets` and shares the canonical heading-marker primitive (`_shared/marker-parser.js`), scanning a superset region so it can never leave a Check-8 leak. Every repair is logged on the run meta. Default OFF = byte-identical output. See **Bracket-leak repair** below.
...
| `repair_bracket_leaks` | boolean | `false` | Turn ON for templates that enable qa-structural `taxonomy_leak_check` | Deterministically deletes stray body-prose bracket markers that would survive the publish strip and force-fail qa-structural Check 8 (e.g. a malformed citation `[#-]`, a bracketed source-pointer `[game pages]`). Preserves heading markers, `[#n]`/`[^n]` references, markdown links, and code. Every repair is logged on the run meta (`bracket_repairs` count + `bracket_repairs_detail`). OFF = byte-identical output. See **Bracket-leak repair** below. |
| `strip_think_blocks` | boolean | `true` | Leave ON. Only turn OFF to deliberately keep raw reasoning-model output | Deterministically removes a leaked `<think>...</think>` chain-of-thought block that a reasoning model wrote as body text (thinking is API-disabled here, so the reasoning surfaces as a block at position 0). Strips only a *closed* block at the start of the draft, never an inline mention mid-prose. **LOUD-FAILS** the entity (no draft published) on an unclosed `<think>` or an all-thinking draft. When it strips, it logs `think_blocks_stripped` + `think_chars_stripped` on the run meta. ON is byte-identical on a clean draft (no block = nothing removed, no meta added). See **`<think>`-leak strip** below. |
...
- **Bracket repair is mechanical only** -- `repair_bracket_leaks` deletes stray *brackets*; it does NOT fix substantive issues (fabrication, missing head-terms, thin sections). A repaired draft is still graded by every QA gate and still routed for human review if any substantive gate fails. Repairs are always logged on the run meta, never silent.
- **`<think>`-leak strip is loud, never a half-strip** -- `strip_think_blocks` (default ON) removes a *closed* `<think>...</think>` block at the start of the draft. An *unclosed* `<think>` (truncated dump) or an *all-thinking* draft (empty after strip) FAILS the entity with an error item and publishes nothing — it never emits a partially-stripped draft. An inline `<think>` mid-prose is left untouched. See **`<think>`-leak strip** below.
...
## Bracket-leak repair (v1.4.0, `repair_bracket_leaks`)

**Why.** Step 6 grades the `.at(-1)` draft — which is this module's output (qa-structural:219-224). qa-structural Check 8 force-fails a draft when a bracket survives the publish strip and would leak to the reader. In practice the writer occasionally emits a **citation-shaped stray marker** in the body — a malformed citation (`[#-]`, `[#−]`, `[#n]`) or a bracketed source-pointer (`[game pages]`, `[hacksawgaming.com]`) — that no reader should see. These are *mechanical* leaks: the fix is a pure string transform. Rejecting an otherwise-true, well-sourced profile because one bracket survived is the wrong trade at scale, so when this option is ON the leak is repaired before the draft is graded (content-pipeline-specs `@94eb952` CALIBRATION_3).

**What it does.** For each line (outside fenced/inline code), it preserves the first heading marker verbatim, then deletes any residual bracket — **with one leading horizontal space** so the prose reads clean (`…sequels [#-] — evidenced` → `…sequels — evidenced`; `…celestial theme [game pages].` → `…celestial theme.`).

**What it never touches** (identical to Check 8's exclusions): valid inline citations `[#n]` / `[n]`, footnote refs `[^n]`, markdown links `[text](url)`, fenced and inline code, and the first heading marker on each heading (`## [Overview]`, `## [Primary Category: slug]` — the bundlers strip these at publish and need them present).

**Never silent.** Every deletion is logged on the run meta as `bracket_repairs` (count) and `bracket_repairs_detail` (`[{ line, bracket }]`), and to the run log. A repaired draft is therefore always distinguishable from a naturally-clean one.

**Can never leave a Check-8 leak (safe, one-directional).** The exclusion predicates are byte-faithful to qa-structural's `findResidualBrackets` (`qa-structural/execute.js:130-156`, deployed `08df8b5`), and it shares the one canonical heading-marker primitive (`_shared/marker-parser.js`). There is exactly one region difference: Check 8 scans the draft *after* stripping the `## [Meta]` section (`removeMetaSection`), whereas the repair scans the whole draft — a strict **superset**. So the repair can only ever *over*-clean (a stray bracket inside the `## [Meta]` block, which markdown/html-output strip at publish anyway, and which is always logged) — it can **never** leave a bracket that Check 8 would flag. Scanning the whole draft avoids replicating `removeMetaSection` as a second lockstep point. The test (`test-bracket-repair.js`) proves loop closure by running the actual deployed qa-structural Check 8 on the repaired draft — residual leaks go 1 → 0.

**Default OFF = byte-identical.** With the option off (or any value other than `true`/`"true"`), the repair pass is skipped entirely and no meta fields are added; output is byte-identical to v1.3.0 (proven in the test against the `08df8b5` baseline). Enable it on templates that turn on qa-structural `taxonomy_leak_check` (e.g. `company-profile-v3`).

## `<think>`-leak strip (v1.5.0, `strip_think_blocks`)

**Why.** Reasoning models here run with thinking **API-disabled** (`thinking:{type:'disabled'}`, see above). With no thinking channel, a reasoning model writes its chain-of-thought as ordinary completion text, and `response.text` flows straight to the published `content_markdown` with no strip. Both observed 4b leaks (content-pipeline-specs Unit 4c §3b, `STEP5_ANALYZER_BAR.md`) were exactly ONE closed `<think>...</think>` block at position 0 (9,038 and 994 words). All 16 published drafts in the surviving prod runs are clean, so the exposure is real but not yet realised — this seals it before any batch.

**Where it runs.** Immediately after the code-fence strip and BEFORE bracket-repair, the marker gate, and every downstream stat (`execute.js`, between the fence-strip and the `repair_bracket_leaks` pass), so the graded/published `.at(-1)` draft is the stripped one. It mirrors the `repair_bracket_leaks` placement and meta pattern.

**Matching precision & false-positives.** The open tag is matched with `/^\s*<think>/i` — the **bare** `<think>` tag (any case) anchored to position 0 after optional leading whitespace — and the first `</think>` closes it. By design it does **not** touch:
- a `<think>` appearing *mid-prose* (never at position 0) — a legitimate inline mention of the characters is left verbatim;
- `<thinking>`, `<think id="…">`, or any tag that is not the bare `<think>`.

The only thing this could false-positive on is a draft whose intended body prose *opens* with a literal `<think>...</think>` as content — which no real article does.

**LOUD-FAIL, never a half-strip.** The strip throws (surfaced by the caller's `try/catch` as an error item with empty `content_markdown`, so nothing is published) on either:
- **unclosed** — an opening `<think>` at position 0 with no `</think>`: a truncated reasoning dump; publishing the half is worse than failing;
- **all-thinking** — the strip leaves nothing but whitespace: the draft was entirely reasoning, a generation failure, not a clean draft.

A silent half-strip is precisely the failure class this project hunts, so it fails loud instead.

**Never silent when it acts.** When a block is stripped, the run meta carries `think_blocks_stripped` (count) and `think_chars_stripped` (block length), and the strip is logged. A stripped draft is therefore always distinguishable from a naturally-clean one.

**Default ON = byte-identical on a clean draft.** A clean draft has no `<think>` block, so the strip removes nothing and — because the meta fields are added only when a block is actually stripped — the entire result (content and meta) is byte-identical to pre-1.5.0. This is proven in `test-think-strip.js` against the `046ae7c` baseline. Only an explicit `false`/`"false"` disables the strip, letting raw model output (including any leaked block) pass through unchanged.

**Ceiling.** It strips only the first block anchored at position 0 (exactly the two observed shapes). A hypothetical second block later in the body is left untouched, to protect the inline-mention precision above; widen to a multi-block scan only if a real multi-block leak is ever observed.
