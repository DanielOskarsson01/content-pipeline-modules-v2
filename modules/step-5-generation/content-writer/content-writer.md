...
This is also the first output that might go directly to a CMS. The markdown output is designed to be copy-pasted or imported into WordPress, Ghost, or any CMS that accepts Markdown. Structured data conversion (JSON for CMS APIs, HTML) is handled by a downstream bundling step, not by the content-writer.
...
**Suggested new tags must be labeled.** When the analysis includes `suggested_new` tags, the writer marks them `[Suggested tag]` in headings so editors can spot them.
...
- **Nothing downstream (currently)** — this is the pipeline's terminal output. Future steps could add media enrichment, CMS publishing, or quality review
...
- Suggested new tags labeled with `[Suggested tag]` in headings
...
- `content_markdown` — the FULL article in Markdown (visible in detail modal only)

**Detail modal:** This is where the article is actually read. The `content_markdown` field renders with `"display": "prose"` in a scrollable area (max-height: 60vh, whitespace-pre-wrap). The card's `content_preview` column shows only a teaser.
...
- Broken markdown formatting — rare but possible. Check for unclosed headers, malformed links
...
- **No web research** — The writer only uses information from the analysis, source content, and reference docs. It cannot look up additional information, check current pricing, or verify facts against live websites
...
- **Many categories stretch word budget** — With 6+ categories at 150–300 words each, the article may exceed the word target. The SEO planner should flag this, but the writer may still overshoot
...
- **Step 9: Monitoring** — tracking published article performance (rankings, traffic, engagement)
